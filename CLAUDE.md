...
rules, audit). Postgres runs on host port **5433** (native PG owns 5432). Runtime AI is
free-tier-first: Gemini 2.5 Flash primary + Groq fallback (`@archai/ai`, provider-agnostic);
set `GEMINI_API_KEY`/`GROQ_API_KEY` in apps/api/.env (UI degrades honestly without them; see
docs/ai-architecture.md). This is unrelated to the models Claude Code uses to develop the repo.
CI workflow lives in `.github/workflows-pending/` until the GitHub token gains the `workflow`
